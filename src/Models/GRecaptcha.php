...
class GRecaptcha
{
	...
	protected static $scripts = [
		array('type' => 'url', 'url' => 'https://www.google.com/recaptcha/api.js?onload=CaptchaCallback&render=explicit'),
		array('type' => 'inline', 'method' => 'captchaGlobals'),
		array('type' => 'url', 'method' => 'startCaptchasScriptUrl'),
	];
	 ...
	 *
	 * @param  array   $options
	 * @param  boolean $render  [if true the captcha is ecod out if not it is only returned as html]
	 * @return string
	 */
	public static function render($options = array(), $render = true)
	{
	...
	public static function renderScript($script)
	{
		...
		if ($script['type'] == 'url') {
			...
			}else if (isset($script['method']) && method_exists(static::class, $script['method'])) {
